import { Educations, Interests, Languages, PersonalSkills, Profile, Projects, TechnicalSkills, Works } from "@schema";
...
const CONTACT_TABLE: Table = {
    widths: ['*'],
    body: [Profile.network.email, Profile.network.whatsapp, Profile.network.website].map(lang => [
        {
            stack: [
...
const SKILL_TABLE: Table = {
    widths: ['*'],
    body: [...TechnicalSkills.map(skill => [{
        stack: [
    ...
    body: [...Works.map(item => ([{
        ...
        table: {
            body: [
                [{
                    columns: [
                        { text: [{ text: `${item.position} `, style: 'mainTitle' }, { text: item.name, link: item.url, color: Color.Link, }], width: 260 },
                        IconLabel('calendar', { text: `${item.startDate} - ${item.endDate}` })
                    ]
                }],
                [{ text: item.summary, alignment: 'justify' }],
                [{ columnGap: 5, columns: item.highlights.map(element => Chip({ label: element })) }]
    ...
    body: [...Educations.map(item => ([{
        ...
        table: {
            body: [
                ...
                [{ text: [{ text: item.area }, { text: ' ' }, { text: item.institution, link: item.url, color: Color.Link }, { text: '.' }], alignment: 'justify' }]
            ]
        }
    }])
    )]
